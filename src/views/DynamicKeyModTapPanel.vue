...
<n-form label-placement="top" label-width="auto" require-mark-placement="right-hanging">
    <n-form-item :label="t('key')">
      <div v-if="dynamic_key_mt.target_keys_location && dynamic_key_mt.target_keys_location.length > 0" class="keyboard no-select" style="height: 54px;">
        <PlainKey v-for="(item,index) in dynamic_key_mt.target_keys_location" :key="index" :width="1" :height="1" :x="index"
    ...
    <n-form-item :label="t('dynamic_key_mt_panel_key_bindings')">
      <div class="keyboard no-select" style="height: 54px;">
        <KeyEditCell v-for="(item,index) in dynamic_key_mt.bindings" :x=index
